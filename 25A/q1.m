...
g = 9.8; 
v_d = 120;  % 无人机速度 m/s
t_deploy = 1.5;  % 无人机飞行时间 s
t_explode_delay = 3.6;  % 投掷后爆炸时间间隔 s
...
% 无人机的速度，朝假目标方向飞
drone_dir = fake_target(1:2) - drone_pos0(1:2);  % xoy平面
drone_dir_norm = norm(drone_dir);
drone_vel = v_d * (drone_dir / drone_dir_norm);  % [vx,vy]
drone_vel = [drone_vel, 0];  % vz=0

% t=t_deploy（1.5s）时抛出烟雾弹，此时无人机的位置
