...
alpha_bound = [-pi/12,pi/12];  % 无人机飞行角度范围
t_deploy_bound = [0,10];  % 无人机飞行时间范围
t_explode_delay_bound = [0,10];  % 投掷后爆炸时间间隔范围 s
...
% 约束条件
constraints1 = @(alpha,t_deploy,drone_speed,t_explode_delay)  abs(missile_vel(1)) * (t_deploy+t_explode_delay) -2200 ...
    - sign(alpha) * drone_speed * cos(alpha) * (t_deploy+t_explode_delay);

% constraints2 = @(t_explode_delay,t_deploy) missile_pos0(3) + missile_vel(3)*(t_explode_delay+t_deploy) ...
%     + 0.5*g*(t_explode_delay)^2- drone_pos0(3);

% 惩罚函数封装
penalty_factor = 3;  % 惩罚因子
fitness_func = @(drone_speed, alpha, t_deploy, t_explode_delay) ...
    aim_func(drone_speed, alpha, t_deploy, t_explode_delay)...
...
shield_time = aim_func(best_drone_speed, best_alpha, best_t_deploy, best_t_explode_delay);
