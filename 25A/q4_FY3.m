...
% 变量范围
drone_speed_bound = [138,140];  % 无人机速度范围
alpha_bound = [-1.570796, 0.000000];  % 无人机飞行角度范围
t_deploy_bound = [20,21];  % 无人机飞行时间范围
t_explode_delay_bound = [1.7,1.8];  % 投掷后爆炸时间间隔范围 s
...
% 约束条件
constraints1 = @(alpha,t_deploy,drone_speed,t_explode_delay)  abs(missile_vel(1)) * (t_deploy+t_explode_delay) - 14000 ...
    - drone_speed * sin(alpha) * (t_deploy+t_explode_delay);

penalty_factor = 1e-2;  % 惩罚因子
fitness_func = @(drone_speed, alpha, t_deploy, t_explode_delay) ...
    aim_func_q4_FY3(drone_speed, alpha, t_deploy, t_explode_delay)...
...
shield_time = aim_func_q4_FY3(best_drone_speed, best_alpha, best_t_deploy, best_t_explode_delay);
