...
D = 8;         % 共需要8个变量
K = 150;       % 最大迭代次数
mutation_particles = 20;  % 每次变异时重置多个粒子


% 无人机所需变量范围
drone_pos0 = [17800,0,1800];  % ===修改位置坐标====
drone_speed_bound = [100,140];  % 无人机速度范围
alpha_bound = [-pi/2,pi/2];  % 无人机飞行角度范围
t_deploy1_bound = [0,40];  % 无人机开始起飞到投掷第1枚烟雾弹
t_deploy2_bound = [0,40];  % 无人机开始起飞到投掷第2枚烟雾弹
t_deploy3_bound = [0,40];  % 无人机开始起飞到投掷第3枚烟雾弹
...
fitness_func = @(drone_speed, alpha,t_deploy1, t_explode_delay1,t_deploy2, t_explode_delay2,t_deploy3, t_explode_delay3) ...
    aim_func_FY2(drone_speed, alpha, t_deploy1, t_explode_delay1,t_deploy2, t_explode_delay2,t_deploy3, t_explode_delay3)...
...
[all_shield_time, shield_time1, shield_time2, shield_time3] = aim_func_FY2_2(best_drone_speed, best_alpha, ...
